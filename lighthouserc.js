module.exports = {
  ci: {
    collect: {
      startServerCommand:
        "pnpm --filter @internal-toolkit/web start --port 4174",
      startServerReadyPattern: "Ready in|listening|started",
      startServerReadyTimeout: 60_000,
      url: [
        // Only audit real HTML pages — never JSON API routes (causes NOT_HTML).
        "http://127.0.0.1:4174/login",
        "http://127.0.0.1:4174/",
      ],
      numberOfRuns: 3,
      settings: {
        preset: "desktop",
        chromeFlags: "--no-sandbox --headless=new --disable-dev-shm-usage --disable-gpu --disable-background-timer-throttling --disable-backgrounding-occluded-windows --disable-renderer-backgrounding",
        maxWaitForLoad: 45000,
      },
    },
    assert: {
      assertions: {
        "categories:performance": ["warn", { minScore: 0.5 }],
        "categories:accessibility": ["warn", { minScore: 0.7 }],
        "categories:best-practices": ["warn", { minScore: 0.7 }],
        "categories:seo": "off",
      },
    },
    upload: {
      target: "temporary-public-storage",
    },
  },
};
